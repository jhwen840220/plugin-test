...
class InlineRawSourcePlugin {
    constructor(options) {
        ...
        this.attachFile = this.options.attachFile || null;
    ...
    apply(compiler) {
        ...
        compiler.hooks.afterCompile.tapAsync('inlineRawSourcePlugin', (compilation, cb) => {
            if (this.notFoundChunkName.length) { 
                compilation.errors.push(`${this.hintMsg}找不到指定的 chunkname: [${this.notFoundChunkName.join(", ")}]`)
                throw new Error(
                    compilation.errors.map(err => err)
                )
            }
            cb();
        });
